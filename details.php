...
<div class="container">    
    <div id="signupbox" style=" margin-top:50px" class="mainbox col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2">
		...
		<div id="div_id_date" class="form-group required">
		    ...
		    <div class="controls col-md-8 ">
		    <input class="input-md textinput textInput form-control" id="date" name="date" placeholder="MM/DD/YYY" style="margin-bottom: 10px" type="text" value="<?php echo $row['date'] ?>" readonly />
		    </div>    
		</div>
...
if ($result->num_rows > 0)
{
	while($row = $result->fetch_assoc())
	{
		...
		echo '		<textarea class="textarea-md textinput textInput form-control" id="id_message" name="message" placeholder="Digite sua mensagem" style="margin-bottom: 10px" type="text" readonly/>' . $row['message'] . '</textarea>';
		echo '	    </div>';
		echo '	</div>';
		echo '</div>';
	}
...
?>
			...
			<label for="id_message" class="control-label col-md-4 requiredField"> <?php echo $band_name ?><span class="asteriskField"></span> </label>
			    <div class="controls col-md-8 ">
